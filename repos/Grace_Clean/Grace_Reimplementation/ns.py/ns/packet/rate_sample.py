...
class Connection:
    ...
    def mark_connection_app_limited(self, packets_in_flight):
        ...
        if self.delivered + packets_in_flight == 0:
            self.is_app_limited = 1
        else:
            self.is_app_limited = self.delivered + packets_in_flight
    ...
    def check_if_application_limited(self, next, mss, packet_in_flight):
        ...
        """
        if (
            self.write_seq - next < mss
            and self.pending_trans == 0
            and not self.is_cwnd_limited
            and self.lost_out <= self.retrans_out
        ):
            if self.delivered + packet_in_flight > 0:
                self.is_app_limited = self.delivered + packet_in_flight
            else:
                self.is_app_limited = 1
