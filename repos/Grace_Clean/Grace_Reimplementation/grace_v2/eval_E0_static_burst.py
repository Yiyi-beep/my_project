"""
Evaluation entry for Fig(E0): static & burst across light/medium/heavy with SP/ECMP/HIER.

Outputs:
  results_E0/E0_runs.csv       # per seed summary rows
  results_E0/E0_windows.jsonl  # per-window stats for time-series/recovery
"""
...
# PPS definitions
BASE_PPS = {"light": 400.0, "medium": 420.0, "heavy": 480.0}
BURST_PPS = {"light": 483.0, "medium": 507.0, "heavy": 580.0}
...
class ScheduledPacketGenerator:
    def __init__(
        ...
        self.metrics = metrics
        self.mode = mode
        self.offset_ms = random.uniform(0.0, offset_ms_range) if offset_ms_range > 0 else 0.0
    ...
    def run(self):
        if self.offset_ms > 0:
            yield self.env.timeout(self.offset_ms)
...
def burst_window_metrics(metrics: MetricsRecorder, burst_start: float | None, burst_end: float | None):
    """Compute burst-window delivery/latency/goodput/service_ratio."""
...
def queue_samples(monitors, measure_start, measure_end):
    samples = []
    for m in monitors:
...
def run_baseline(
    ...
    seed: int,
    offset_ms_range: float = 100.0,
    burst_start_ms: float | None = None,
    burst_end_ms: float | None = None,
    arrival_process: str = "poisson",
):
    set_seed(seed)
    ...
    for fid, (src, dst) in enumerate(config.FLOW_PAIRS):
        gen = ScheduledPacketGenerator(
            ...
            metrics=metrics,
            mode=mode_label,
            offset_ms_range=offset_ms_range,
    ...
    burst_extra = burst_window_metrics(metrics, burst_start_ms, burst_end_ms)
    ...
    return {
        ...
        "mode": mode_label,
        "variant": variant,
        "base_pps": BASE_PPS[mode_label],
        "burst_pps": BURST_PPS[mode_label] if variant == "burst" else BASE_PPS[mode_label],
...
def run_hier(
    ...
    set_seed(seed)
    model_mode = mode_label  # use same mode for bandit context
    env = simpy.Environment()
    metrics = MetricsRecorder(window_ms=WINDOW_MS)
    ...
    cap_per_mode = sim.cap_profile.get(model_mode, {})
    ...
    last_context: Dict[int, List[float]] = {
        fid: sim._make_context(model_mode, None, cap_per_mode) for fid in controllers
    }
    ...
    for w in range(NUM_WINDOWS):
        ...
        for fid in controllers:
            ...
            ctx = last_context.get(fid, sim._make_context(model_mode, None, cap_per_mode))
            if use_bandit:
              cid, _ = sim.bandit.select(pair, model_mode, ctx, k_eff)
              cid = min(cid, max_arm)
            else:
              cid = 0  # ✅ candidate baseline 不做 bandit，这里给个固定值即可（router 会忽略）
        ...
        for fid in controllers:
            ...
            last_context[fid] = sim._make_context(model_mode, summary, cap_per_mode)
    ...
    if use_bandit:
      ...
      windows = aggregate_window_stats(metrics)  # ✅ candidate 不记录 bandit/cluster 曲线
...
def main():
    parser = argparse.ArgumentParser(description="E0 static/burst evaluation for SP/ECMP/HIER.")
    parser.add_argument("--seeds", type=int, nargs="+", default=list(range(10)))
    parser.add_argument("--outdir", type=Path, default=Path("results_E0"))
    parser.add_argument("--offset-ms-range", type=float, default=0.0, help="Random start offset per flow in ms (0 to disable)")
    parser.add_argument(
        "--burst-jitter-ms",
        type=float,
        default=0.0,
        help="Uniform jitter (+/-) applied to burst start time (0 to disable)",
    )
    parser.add_argument(
        "--arrival-process",
        type=str,
        choices=["poisson", "const"],
        default="poisson",
        help="Per-flow arrival process for packet generation",
    )
    args = parser.parse_args()
    if args.offset_ms_range >= MEASURE_START:
        raise ValueError(f"offset_ms_range must be < MEASURE_START ({MEASURE_START} ms) to keep offered load consistent.")
    ...
    for mode in ["light", "medium", "heavy"]:
        base = BASE_PPS[mode]
        burst = BURST_PPS[mode]
        schedules[(mode, "static")] = [(0.0, MEASURE_END, base)]
        schedules[(mode, "burst")] = (base, burst)
    ...
    runs = []
    windows_log = outdir / "E0_windows.jsonl"
    if windows_log.exists():
        windows_log.unlink()

    for mode in ["light", "medium", "heavy"]:
        for variant in ["static", "burst"]:
            sched = schedules[(mode, variant)]
            for seed in seeds:
                # Resolve burst jitter per seed/mode if variant == burst (bind to seed for reproducibility)
                if variant == "burst":
                    rng = random.Random(seed * 100 + ["light", "medium", "heavy"].index(mode))
                    jitter = rng.uniform(-args.burst_jitter_ms, args.burst_jitter_ms)
                    bstart = max(MEASURE_START, BURST_START_BASE + jitter)
                    bend = min(MEASURE_END, bstart + BURST_DURATION)
                    assert bend > bstart, "Burst window collapsed; adjust jitter/DURATION."
                    sched_use = [
                        (0.0, bstart, sched[0]),
                        (bstart, bend, sched[1]),
                        (bend, MEASURE_END, sched[0]),
                    ]
                else:
                    bstart = BURST_START_BASE
                    bend = BURST_START_BASE + BURST_DURATION
                    sched_use = sched  # already a full schedule with (start,end,pps)
                ...
                for policy in ["sp", "ecmp"]:
                    res = run_baseline(
                        ...
                        sched_use,
                        seed,
                        offset_ms_range=args.offset_ms_range,
                        burst_start_ms=bstart if variant == "burst" else None,
                        burst_end_ms=bend if variant == "burst" else None,
    ...
    with runs_path.open("w", newline="") as f:
        ...
        for r in runs:
            ...
            row["burst_jitter_ms"] = args.burst_jitter_ms if row["variant"] == "burst" else 0.0
            writer.writerow(row)

    print(f"[Eval] runs -> {runs_path}")
    print(f"[Eval] windows -> {windows_log}")
