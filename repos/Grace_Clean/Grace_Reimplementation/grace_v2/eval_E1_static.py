"""
E1 static scalability: scale concurrent flows (F=3/6/9/12) under static load (no burst) for light/medium/heavy.

Outputs:
  results_E1_static/E1_static_runs.csv
  results_E1_static/E1_static_windows.jsonl
"""
...
assert abs((MEASURE_END - MEASURE_START) / WINDOW_MS - NUM_WINDOWS) < 1e-6
...
def run_baseline(policy: str, flows: List[Tuple[int, int]], mode: str, pps: float, seed: int):
    set_seed(seed)
...
def run_hier(flows: List[Tuple[int, int]], mode: str, pps: float, seed: int, sim: HierarchicalRoutingSimulator):
    set_seed(seed)
    env = simpy.Environment()
    metrics = MetricsRecorder(window_ms=WINDOW_MS)
    ...
    cap_per_mode = sim.cap_profile.get(mode, {})
    ...
    last_context: Dict[int, List[float]] = {
        fid: sim._make_context(mode, None, cap_per_mode) for fid in controllers
    }
    ...
    for w in range(NUM_WINDOWS):
        for fid, (src, dst) in enumerate(flows):
            ...
            ctx = last_context.get(fid, sim._make_context(mode, None, cap_per_mode))
            cid, _ = sim.bandit.select(pair, mode, ctx, k_eff)
            cid = min(cid, max_arm)
        ...
        for fid in controllers:
            ...
            last_context[fid] = sim._make_context(mode, summary, cap_per_mode)
    ...
    for fid_stats in metrics.window_stats.values():
        for w, stats in fid_stats.items():
            if 0 <= w < NUM_WINDOWS:
                total_bits += stats.delivered_bits
    goodput_mbps = (total_bits / (config.MEASURE_MS / 1000.0)) / 1e6 if total_bits else 0.0
    windows = aggregate_window_stats(metrics, reward_trace=reward_trace)
...
def main():
    parser = argparse.ArgumentParser(description="E1 static scalability (no burst) for SP/ECMP/HIER.")
    ...
    runs = []
    windows_log = outdir / "E1_static_windows.jsonl"
        ...
        for mode in modes:
            ...
            for seed in seeds:
                for policy in ["sp", "ecmp"]:
                    res = run_baseline(policy, flows, mode, pps, seed)
                    runs.append(res)
                    for wrec in res["windows"]:
                        append_jsonl(
                            windows_log,
                            {"policy": res["policy"], "flows": len(flows), "mode": mode, "seed": seed, **wrec},
                        )
                res = run_hier(flows, mode, pps, seed, sim)
                runs.append(res)
                for wrec in res["windows"]:
                    append_jsonl(
                        windows_log,
    ...
    runs_path = outdir / "E1_static_runs.csv"
    ...
    print(f"[Eval E1 Static] runs -> {runs_path}")
    print(f"[Eval E1 Static] windows -> {windows_log}")
